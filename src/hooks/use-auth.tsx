...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const needsProfileCompletion = profileSettled && (!profile || profile.name === profile.phone);
  ...
  const sendOtp = useCallback(async (phone: string) => {
    ...
    const { data, error } = await supabase.functions.invoke<SendResponse>("auth-send-otp", {
      body: { phone: e164 },
    });
  ...
  const verifyOtp = useCallback(async (phone: string, otp: string) => {
    ...
    const { data, error } = await supabase.functions.invoke<VerifyResponse>("auth-verify-otp", {
      body: { phone: e164, otp },
    });
