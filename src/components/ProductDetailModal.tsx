...
export function ProductDetailModal({
  ...
  const [selectedStartDate, setSelectedStartDate] = useState<Date>(minStartDate);
  const [showCalendar, setShowCalendar] = useState(false);
  const [calendarViewMonth, setCalendarViewMonth] = useState(startOfMonth(minStartDate));
  const [imageFailed, setImageFailed] = useState(false);
  const [isCloseClicked, setIsCloseClicked] = useState(false);
  const [displayedDeliveryType, setDisplayedDeliveryType] = useState<"one_time" | "subscription">(
    "one_time",
  );
  ...
  const existingSubscriptionItem = useMemo(
    () =>
      product
        ? cart.find(
            (item) => item.product_id === product.id && item.delivery_type === "subscription",
          )
        : undefined,
    [cart, product],
  );
  ...
  const selectedDurationOption = useMemo(
    () =>
      subscriptionConfig && selectedDuration
        ? subscriptionConfig.durations.find((d) => d.duration_days === selectedDuration)
        : null,
    [subscriptionConfig, selectedDuration],
  );
  ...
  const isCalendarDeliveryDay = (day: Date) => {
    ...
    if (isBefore(day, selectedStartDate) && !isSameDay(day, selectedStartDate)) return false;
    if (selectedDuration) {
      const lastDeliveryDate = addDays(selectedStartDate, (selectedDuration - 1) * interval);
  ...
  useEffect(() => {
    if (open && product && productDetail) {
      ...
      const existingOneTime = cart.find(
        (item) => item.product_id === product.id && item.delivery_type === "one_time",
      );
                  ...
                  <div className="relative flex bg-muted/60 rounded-full p-1 border border-border/40 shadow-inner">
                    ...
                    >
                      ...
                      <Badge className="text-[8px] bg-emerald-600 text-white border-0 px-1.5 py-0 leading-tight font-semibold">
                        -{Math.max(...subscriptionConfig.durations.map((d) => d.discount_percent))}%
                      </Badge>
                    </button>
                  </div>
                </div>
                        ...
                        <div className="grid grid-cols-7 gap-1">
                          {(() => {
                            ...
                            return cells.map((day, i) => {
                              ...
                              const outOfRange =
                                isBefore(day, minStartDate) || isBefore(maxStartDate, day);
                              return (
                                <button
                                  key={i}
                                  type="button"
                  ...
                  {selectedDuration && (
                    <div className="flex items-center justify-between text-xs bg-gradient-to-r from-primary/8 to-accent/8 px-3.5 py-2.5 rounded-xl border border-primary/15">
                      <span className="text-muted-foreground">
                        {deliveryCount} deliveries · ₹{selectedVariant?.price.toFixed(2) || "0.00"}{" "}
                        each
                      </span>
