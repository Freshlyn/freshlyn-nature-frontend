...
export function ProductDetailModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto p-0 flex flex-col rounded-2xl shadow-2xl [&>button]:hidden">
        <div className="relative h-48 md:h-56 overflow-hidden bg-gradient-to-br from-primary/40 via-primary/15 to-accent/30 shrink-0">
          {!imageFailed && (
            <img
          ...
          {imageFailed && (
            <div className="absolute inset-0 flex items-center justify-center">
              <ImageOff size={40} className="text-white/40" />
            </div>
          )}
          ...
          <div className="absolute bottom-4 left-5 right-5">
            <Badge
              variant="secondary"
              className="bg-white text-foreground capitalize mb-2 shadow-sm"
            >
              {product.category}
            </Badge>
            <DialogTitle className="text-2xl md:text-3xl text-white drop-shadow-lg font-display">
              {product.name}
            </DialogTitle>
          </div>
        </div>
        ...
        <div className="p-5 md:p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {variants.map((variant) => (
                  <button
                    ...
                    className={`relative p-3 rounded-xl border text-center transition-all active:scale-95 ${
                  ...
                  >
                    {selectedVariantId === variant.id && (
                      <CheckCircle2
                        size={16}
                        className="absolute -top-1.5 -right-1.5 text-primary bg-white rounded-full"
                      />
                    )}
          ...
          {hasSubscription && subscriptionConfig && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-2">
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center ${deliveryType === "one_time" ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}
                  >
                    <ShoppingBag size={18} />
                  </div>
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center ${deliveryType === "subscription" ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}
                  >
                    <Repeat size={18} />
                  </div>
                  <div className="flex-1">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center bg-muted/50 rounded-xl p-1 border border-border/50">
                  ...
                  <span
                    className="font-bold w-12 text-center text-lg"
          ...
          <div className="-mx-5 md:-mx-6 -mb-5 md:-mb-6 px-5 md:px-6 pt-4 pb-5 md:pb-6 bg-muted/40 border-t border-border/50">
